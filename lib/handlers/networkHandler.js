...
const emitXHREvent = p => {
    ...
    if (!(requestPromises && requestPromises[p.requestId])) {
        ...
        logEvent('Request started: ' + JSON.stringify(log));
        let resolve;
        eventHandler.emit('xhrEvent', new Promise((r) => { resolve = r; }));
...
const resolveXHREvent = p => {
    if (requestPromises && requestPromises[p.requestId]) {
        logEvent('Request resolved: ' + p.requestId);
...
const handleInterceptor = p => {
    ...
    const matches = interceptors.filter((interceptor) => {
        ...
        var matches = p.request.url.match(re);
        if ( matches === null || matches.length <= 0) {
            return decodeURI(p.request.url).match(re);
        }
        return matches;
    });
    ...
    if(matchesLen){
        if(matchesLen > 1) {
            ...
            console.warn(`WARNING: More than one intercept [${matchesURL}] found for request "${p.request.url}".\n Applying: intercept("${matches[matchesLen-1].requestUrl}", "${matches[matchesLen-1].action}")`);
        }
        ...
        if (!interceptor.action) { options.errorReason = defaultErrorReason;}
            ...
            if (!/^https?:\/\//i.test(interceptor.action) && !/^file/i.test(interceptor.action)) interceptor.action = 'http://' + interceptor.action;
...
module.exports = { addInterceptor, resetInterceptors, setNetworkEmulation, handleInterceptor, resetPromises };